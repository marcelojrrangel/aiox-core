...
export class LanguageSupport {
    ...
    detectLanguageByName(name) {
        const lower = name.toLowerCase();
        const found = LANGUAGE_CATALOG.find((l) => l.name.toLowerCase() === lower || l.extension === `.${lower}`);
        if (found)
            return found;
    ...
    buildLanguagePrompt(basePrompt, language) {
        const info = this.detectLanguageByName(language);
        return `${info.systemPrompt}\n\n${basePrompt}`;
    }
}
