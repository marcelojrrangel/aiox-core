...
async function startChatSession(model, useStream) {
    const history = [
...
async function processUserInput(model, input, history, useStream) {
    history.push({ role: 'user', content: input });
    try {
        if (useStream) {
            process.stdout.write(chalk.green('\n  CodeMin: '));
            let fullResponse = '';
            const stream = ollamaClient.generateStream(model, history, // includes the user message just pushed above
...
function buildGeneratePrompt(description) {
