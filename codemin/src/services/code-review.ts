import type { CodeReviewReport, CodeReviewIssue } from '../types/index.js';
...
export class CodeReviewService {
  ...
  async review(code: string, language: string): Promise<CodeReviewReport> {
...
Relatório de revisão (formato JSON):`;
  ...
  private parseReviewReport(content: string): CodeReviewReport {
    try {
      const jsonMatch = content.match(/\{[\s\S]*\}/);
      if (jsonMatch) {
        const parsed = JSON.parse(jsonMatch[0]) as CodeReviewReport;
        return {
