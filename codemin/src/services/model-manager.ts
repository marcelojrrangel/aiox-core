...
import { executeAndCheck, execSyncSafe } from '../utils/shell.js';
...
const MODEL_DETAILS: Record<string, ModelCatalogEntry> = {
  'qwen2.5-coder:7b': {
    id: 'qwen2.5-coder:7b',
    name: 'Qwen2.5-Coder 7B Instruct',
    size: '4.7 GB',
...
function getActiveModelPath(): string {
...
function readActiveModel(): string {
  try {
    ...
    if (existsSync(path)) {
      const data = JSON.parse(readFileSync(path, 'utf-8')) as { model: string };
...
function writeActiveModel(model: string): void {
  const path = getActiveModelPath();
...
export class ModelManager {
  ...
  async resolve(modelName?: string): Promise<ModelConfig> {
  ...
  async listInstalledModels(): Promise<{ name: string; size: number; details?: ModelCatalogEntry }[]> {
    try {
  ...
  formatModelList(installed: { name: string; size: number; details?: ModelCatalogEntry }[]): string {
    const lines: string[] = [];
  ...
  async detectOllamaInstall(): Promise<{ installed: boolean; version?: string }> {
  ...
  async detectOllamaServer(): Promise<boolean> {
